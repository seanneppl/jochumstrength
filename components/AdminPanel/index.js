...
const AdminPanel = ({ loading, usersList }) => {
  ...
  return (
    <>
      <Sidebar menu={<UserList setCurrentUser={setCurrentUser} usersList={usersList} current={currentUser} />}>
...
const UserList = withFirebase(withRouter(UserListBase));
  ...
  const onSendGroupMessage = (e) => {
    ...
    if (text !== "" && sendable) {
      ...
      groupMessageList.forEach(key => {
        ...
        firebase.user(key).child("unread").push(messageObject);
      })
      handleClose();
    }
  }
